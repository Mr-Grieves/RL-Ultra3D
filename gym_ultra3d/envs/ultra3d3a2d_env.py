...
class Ultra3DEnv3A2D(gym.Env):
    ...
    def _take_action(self, action):
        ...
        if  (self.curr_th == -1. and act[0] == -1) or \
            (self.curr_th == 1.  and act[0] ==  1) or \
            (self.curr_ph == -1. and act[1] == -1) or \
            (self.curr_ph == 1.  and act[1] ==  1) or \
            (self.curr_ps == -1. and act[2] == -1) or \
            (self.curr_ps == 1.  and act[2] ==  1) or \
            (self.curr_dx == -1. and act[3] == -1) or \
            (self.curr_dx ==  1. and act[3] ==  1) or \
            (self.curr_dy == -1. and act[4] == -1) or \
            (self.curr_dy ==  1. and act[4] ==  1):
            self.oob = True
        ...
        # Update phi
        self.curr_ps = self.curr_ps + self.alpha*act[2]
        self.curr_ps = min(1, max(-1, self.curr_ph))  # Threshold phi

        # Update ds
        self.curr_dx = self.curr_dx + self.alpha*act[3]
        self.curr_dy = self.curr_dy + self.alpha*act[4]
        self.curr_dx = min(1, max(-1, self.curr_dx)) # Threshold dx
        self.curr_dy = min(1, max(-1, self.curr_dy)) # Threshold dy

        self.curr_slice = self.get_slice(self.curr_th, self.curr_ph, self.curr_ps, self.curr_dx, self.curr_dy)
    ...
    def reset(self):
        ...
        self.curr_ps = random.uniform(-1, 1)
    ...
    def force_reset(self, theta, phi, psi, dx, dy):
        # Random init of state
        self.curr_th = theta
        self.curr_ph = phi
        self.curr_ps = psi
    ...
    def get_slice(self, theta_n, phi_n, psi_n, dx_n, dy_n):
        theta = math.radians(theta_n*THETA_MAX)
        phi = math.radians(phi_n*PHI_MAX)
        psi = math.radians(psi_n*PSI_MAX)
        dx = dx_n*self.x0/2  # +/- 200 pixels
        dy = dy_n*self.y0/2  # +/- 350 pixels

        # --- 1: Get bounding box dims ---
        h1, h2, z_min, z_max = self.get_bounding_box(theta=theta, phi=phi, psi=psi, dx=dx, dy=dy)
        w = self.mask_size
        h = self.mask_size
